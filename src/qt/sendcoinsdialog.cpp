...
void SendCoinsDialog::setModel(WalletModel *_model)
{
    ...
    if(_model && _model->getOptionsModel())
    {
        ...
        for (const int n : confTargets) {
            ui->confTargetSelector->addItem(tr("%1 (%2 blocks)").arg(GUIUtil::formatNiceTimeOffset(n*Params().GetConsensus().xdiskIP008TargetSpacing)).arg(n));
        }
...
void SendCoinsDialog::on_genBindDataButton_clicked()
{
    ...
    int activeHeight = std::max(nSpendHeight - 1, Params().GetConsensus().xdiskIP006Height);
