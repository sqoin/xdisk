    ...
    int nSpendHeight, CAmount& txfee, const CAccountID& generatorAccountID, bool fStrictCheckLimit, const Consensus::Params& params)
{
    ...
    if (tx.IsUniform() && nSpendHeight >= params.xdiskIP006Height) {
    ...
    if (tx.IsUniform() && nSpendHeight >= params.xdiskIP006CheckRelayHeight) {
        if (tx.vin.size() == 1 && tx.vout.size() == 1) {
            // Unbind & Withdraw
            const Coin& coin = inputs.AccessCoin(tx.vin[0].prevout);
            if (!coin.extraData && nSpendHeight >= params.xdiskIP007Height)
            ...
            if (nSpendHeight >= params.xdiskIP007Height && !payload)
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-invaliduniform-type");

            // Bind plotter
            if (payload && payload->type == DATACARRIER_TYPE_BINDPLOTTER && nSpendHeight >= params.xdiskIP006LimitBindPlotterHeight) {
                // Low fee
...
int Consensus::GetBindPlotterLimitHeight(int nBindHeight, const CBindPlotterInfo& lastBindInfo, const Consensus::Params& params)
{
    ...
    if (nBindHeight < params.xdiskIP006LimitBindPlotterHeight)
        return std::max(params.xdiskIP006Height, lastBindInfo.nHeight + 1);

    // Checking range [nEvalBeginHeight, nEvalEndHeight]
    const int nEvalBeginHeight = std::max(nBindHeight - params.nCapacityEvalWindow, params.xdiskIP001PreMiningEndHeight + 1);
...
int Consensus::GetUnbindPlotterLimitHeight(const CBindPlotterInfo& bindInfo, const CCoinsViewCache& inputs, const Consensus::Params& params)
{
    ...
    if (nSpendHeight < params.xdiskIP006CheckRelayHeight)
        return std::max(params.xdiskIP006Height, bindInfo.nHeight + 1);

    // Checking range [nEvalBeginHeight, nEvalEndHeight]
    const int nEvalBeginHeight = std::max(nSpendHeight - params.nCapacityEvalWindow, params.xdiskIP001PreMiningEndHeight + 1);
    ...
    } else if (nSpendHeight < params.xdiskIP007Height) {
