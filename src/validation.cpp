                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s xdisk additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    int halvings;
    if (nHeight < consensusParams.xdiskIP008Height) {
        halvings = nHeight / (consensusParams.nSubsidyHalvingInterval * 600 / consensusParams.xdiskIP001TargetSpacing);
    } else {
        ...
        int nEqualHeight = consensusParams.xdiskIP008Height * consensusParams.xdiskIP001TargetSpacing / consensusParams.xdiskIP008TargetSpacing;
        halvings = (nHeight - consensusParams.xdiskIP008Height + nEqualHeight) / (consensusParams.nSubsidyHalvingInterval * 600 / consensusParams.xdiskIP008TargetSpacing);
    }
    if (halvings >= 64) {
    ...
    const CCoinsViewCache& view, const Consensus::Params& consensusParams)
{
    ...
    if (nHeight <= consensusParams.xdiskIP001PreMiningEndHeight) {
        // Fund pre-mining
        reward.fund = nSubsidy + nFees;
    } else if (nHeight <= consensusParams.xdiskIP001FundZeroLastHeight) {
        // All to miner
        reward.miner = nSubsidy + nFees;
    } else if (nHeight < consensusParams.xdiskIP006Height) {
        // xdiskIP004
        ...
        //
        // See https://xdisk.org/wiki/developer/xdisk004-soft-fork-for-multimining
        //
        ...
        if (accountBalance >= miningRequireBalance) {
            // Full mortgage
            reward.fund = (nSubsidy * consensusParams.xdiskIP001FundRoyaltyForFullMortgage) / 1000;
            if (nHeight < consensusParams.xdiskIP004AbandonHeight && accountBalance < miningRequireBalanceAtOldConsensus) {
                // Old consensus => fund
                reward.miner0 = (nSubsidy * consensusParams.xdiskIP001FundRoyaltyForLowMortgage) / 1000;
            }
        } else {
            // Low mortgage
            reward.fund = (nSubsidy * consensusParams.xdiskIP001FundRoyaltyForLowMortgage) / 1000;
            reward.fUnconditional = true;
        }
    } else if (nHeight < consensusParams.xdiskIP008Height) {
        // xdiskIP006
        ...
        if (accountBalance - balanceLoan + balanceBorrow >= miningRequireBalance) {
            ...
            // Low mortgage
            int nPeriod = (nHeight - consensusParams.xdiskIP008Height) / consensusParams.xdiskIP008FundRoyaltyDecreasePeriodForLowMortgage;
            int fundRatio = consensusParams.xdiskIP008FundRoyaltyForLowMortgage - consensusParams.xdiskIP008FundRoyaltyDecreaseForLowMortgage * nPeriod;
            if (fundRatio < consensusParams.xdiskIP001FundRoyaltyForFullMortgage)
                fundRatio = consensusParams.xdiskIP001FundRoyaltyForFullMortgage;
            assert(fundRatio <= consensusParams.xdiskIP001FundRoyaltyForLowMortgage);
            reward.fund = (nSubsidy * fundRatio) / 1000;
            reward.accumulate -= (nSubsidy * (consensusParams.xdiskIP001FundRoyaltyForLowMortgage - fundRatio)) / 1000;
            reward.fUnconditional = true;
        }
    }
...
CAmount GetLowMortgageAccumulateSubsidy(const CBlockIndex* pindexPrev, const Consensus::Params& consensusParams)
{
    ...
    for (const CBlockIndex* pindex = pindexPrev;
            (pindex != nullptr) && (pindex->nStatus & BLOCK_UNCONDITIONAL) && (pindex->nHeight >= consensusParams.xdiskIP008Height);
            pindex = pindex->pprev) {
        int nPeriod = (pindex->nHeight - consensusParams.xdiskIP008Height) / consensusParams.xdiskIP008FundRoyaltyDecreasePeriodForLowMortgage;
        int fundRatio = consensusParams.xdiskIP008FundRoyaltyForLowMortgage - consensusParams.xdiskIP008FundRoyaltyDecreaseForLowMortgage * nPeriod;
        if (fundRatio < consensusParams.xdiskIP001FundRoyaltyForFullMortgage)
            fundRatio = consensusParams.xdiskIP001FundRoyaltyForFullMortgage;
        assert(fundRatio <= consensusParams.xdiskIP001FundRoyaltyForLowMortgage);
        accumulate += (GetBlockSubsidy(pindex->nHeight, consensusParams) * (consensusParams.xdiskIP001FundRoyaltyForLowMortgage - fundRatio)) / 1000;
    }
    return accumulate;
}
...
{
    if (nHeight <= consensusParams.xdiskIP001FundZeroLastHeight) {
        return 0;
    } else {
        return (GetBlockSubsidy(nHeight, consensusParams) * consensusParams.xdiskIP001FundRoyaltyForFullMortgage) / 1000;
    }
}
...
CAmount GetLowMortgageFundRoyalty(int nHeight, const Consensus::Params& consensusParams)
{
    if (nHeight <= consensusParams.xdiskIP001FundZeroLastHeight) {
        ...
        return (GetBlockSubsidy(nHeight, consensusParams) * consensusParams.xdiskIP001FundRoyaltyForLowMortgage) / 1000;
    } else {
        int nPeriod = (nHeight - consensusParams.xdiskIP008Height) / consensusParams.xdiskIP008FundRoyaltyDecreasePeriodForLowMortgage;
        int fundRatio = consensusParams.xdiskIP008FundRoyaltyForLowMortgage - consensusParams.xdiskIP008FundRoyaltyDecreaseForLowMortgage * nPeriod;
        if (fundRatio < consensusParams.xdiskIP001FundRoyaltyForFullMortgage)
...
int GetFullMortgageFundRoyaltyRatio(int nHeight, const Consensus::Params& consensusParams)
{
    if (nHeight <= consensusParams.xdiskIP001FundZeroLastHeight) {
        ...
        return consensusParams.xdiskIP001FundRoyaltyForFullMortgage;
    }
}
...
int GetLowMortgageFundRoyaltyRatio(int nHeight, const Consensus::Params& consensusParams)
{
    if (nHeight <= consensusParams.xdiskIP001FundZeroLastHeight) {
        ...
        return consensusParams.xdiskIP001FundRoyaltyForLowMortgage;
    } else {
        int nPeriod = (nHeight - consensusParams.xdiskIP008Height) / consensusParams.xdiskIP008FundRoyaltyDecreasePeriodForLowMortgage;
        int fundRatio = consensusParams.xdiskIP008FundRoyaltyForLowMortgage - consensusParams.xdiskIP008FundRoyaltyDecreaseForLowMortgage * nPeriod;
        if (fundRatio < consensusParams.xdiskIP001FundRoyaltyForFullMortgage)
...
int32_t ComputeBlockVersion(const CBlockIndex* pindexPrev, const Consensus::Params& params)
{
    ...
    if (pindexPrev != nullptr && pindexPrev->nHeight + 1 >= params.xdiskIP004Height && pindexPrev->nHeight + 1 < params.xdiskIP006Height)
        nVersion |= VERSIONBITS_xdiskV2_BITS;
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Check bind
    if (pindex->nHeight >= chainparams.GetConsensus().xdiskIP006BindPlotterActiveHeight && !view.HaveActiveBindPlotter(pindex->generatorAccountID, pindex->nPlotterId)) {
    ...
    if (pindex->nHeight >= chainparams.GetConsensus().xdiskIP006Height) {
        ...
        if (blockReward.fund != 0) {
            ...
            if (!chainparams.GetConsensus().xdiskFundAddressPool.count(address))
                return state.DoS(100,
        ...
        if (pindex->nVersion & VERSIONBITS_xdiskV2_BITS) {
            ...
            if (pindex->nHeight >= chainparams.GetConsensus().xdiskIP004AbandonHeight)
                return state.DoS(100,
            ...
            if (fund != 0) {
                // Require pay to fund
                // For xdiskIP004: [0] => miner, [1] => fund, [2] => miner-append
                // Check output size
                if (block.vtx[0]->vout.size() < 2)
                    return state.DoS(100,
                ...
                if (block.vtx[0]->vout[1].nValue < fund) {
                    if (pindex->nHeight >= chainparams.GetConsensus().xdiskIP004Height) {
                        // Bug, accept corruption pay for fund. See https://xdisk.org/wiki/xdiskIP/004#bad-blocks
    ...
    if (pindex->nHeight >= chainparams.GetConsensus().xdiskIP008Height && blockReward.fUnconditional) {
...
static bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, bool fCheckWork)
{
    ...
    // Block signature
    if (pindexPrev->nHeight + 1 >= chainparams.GetConsensus().xdiskIP007Height) {
