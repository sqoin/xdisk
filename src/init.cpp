...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/xdisk/xdisk>";
    const std::string URL_WEBSITE = "<https://xdisk.org>";
...
public:
    ...
    static void BlockTipChanged(CBlockChainInitTrace *tracer, bool initialSync, const CBlockIndex *pBlockIndex) {
        ...
        int startMiningHeight = Params().GetConsensus().xdiskIP001PreMiningEndHeight;
        ...
        do {
            try {
                ...
                if (!is_coinsview_empty) {
                    ...
                    if (fCoinDBUpgraded && chainActive.Height() >= chainparams.GetConsensus().xdiskIP007Height) {
                        CBlockChainInitTrace upgradeTracer(_("Upgrading UTXO database"), chainActive.Tip());

                        CValidationState state;
                        {
                            LOCK(cs_main);
                            CBlockIndex *pindex = chainActive[chainparams.GetConsensus().xdiskIP007Height];
