...
const uint64_t xdisk_BASE_TARGET_240 = 18325193796ull;

// 4398046511104 / 300 = 14660155037
const uint64_t xdisk_BASE_TARGET_300 = 14660155037ull;

// 4398046511104 / 180 = 24433591728
const uint64_t xdisk_BASE_TARGET_180 = 24433591728ull;
...
{
    // Fund
    if (prevBlockIndex.nHeight + 1 <= params.xdiskIP001PreMiningEndHeight)
        return 0;

    // xdiskIP006 disallow plotter 0
    if (block.nPlotterId == 0 && prevBlockIndex.nHeight + 1 >= params.xdiskIP006Height)
...
uint64_t CalculateBaseTarget(const CBlockIndex& prevBlockIndex, const CBlockHeader& block, const Consensus::Params& params)
{
    ...
    if (nHeight < params.xdiskIP001PreMiningEndHeight + 4) {
        // genesis block & pre-mining block & const block
        return xdisk_BASE_TARGET_240;
    } else if (nHeight < params.xdiskIP001PreMiningEndHeight + 2700 && nHeight < params.xdiskIP006Height) {
        // [N-1,N-2,N-3,N-4]
        const int N = 4;
        ...
        uint64_t newBaseTarget = (curBaseTarget * diffTime) / (params.xdiskIP001TargetSpacing * 4);
        if (newBaseTarget > xdisk_BASE_TARGET_240) {
            newBaseTarget = xdisk_BASE_TARGET_240;
        }
    ...
    } else if (nHeight < params.xdiskIP006Height) {
        // [N-1,N-2,...,N-24,N-25]
        const int N = 24;
        const CBlockIndex *pLastindex = &prevBlockIndex;
        uint64_t avgBaseTarget = pLastindex->nBaseTarget;
        ...
        int64_t diffTime = block.GetBlockTime() - pLastindex->GetBlockTime();
        int64_t targetTimespan = params.xdiskIP001TargetSpacing * N;
        ...
        return newBaseTarget;
    } else if (nHeight < params.xdiskIP008Height) {
        // [N-1,N-2,...,N-287,N-288]
        const int N = 288 - 1;
        const CBlockIndex *pLastindex = &prevBlockIndex;
        uint64_t avgBaseTarget = pLastindex->nBaseTarget;
        ...
        int64_t targetTimespan = params.xdiskIP001TargetSpacing * N; // Bug: targetTimespan small
        if (diffTime < targetTimespan / 2) {
            diffTime = targetTimespan / 2;
        }
        if (diffTime > targetTimespan * 2) {
        ...
        // Use average BaseTarget for first xdiskIP008 genesis block
        ...
        // 300 to 180
        const arith_uint256 bt180(xdisk_BASE_TARGET_180), bt300(xdisk_BASE_TARGET_300);
        arith_uint256 bt(avgBaseTarget);
        bt *= bt180;
        bt /= bt300;
        return bt.GetLow64();
    } else if (nHeight < params.xdiskIP008Height + 4) {
        return prevBlockIndex.nBaseTarget;
    } else if (nHeight < params.xdiskIP008Height + 80) {
        // [N-1,N-2,N-3,N-4]
        const int N = 4;
        const CBlockIndex *pLastindex = &prevBlockIndex;
        uint64_t avgBaseTarget = pLastindex->nBaseTarget;
        ...
        uint64_t curBaseTarget = avgBaseTarget;
        uint64_t newBaseTarget = (curBaseTarget * diffTime) / (params.xdiskIP008TargetSpacing * 4);
        if (newBaseTarget > xdisk_BASE_TARGET_180) {
            newBaseTarget = xdisk_BASE_TARGET_180;
        }
        if (newBaseTarget < (curBaseTarget * 9 / 10)) {
            newBaseTarget = curBaseTarget * 9 / 10;
        }
        ...
        int64_t diffTime = block.GetBlockTime() - pLastindex->GetBlockTime() - static_cast<int64_t>(N) * 1;
        int64_t targetTimespan = params.xdiskIP008TargetSpacing * N;
        if (diffTime < targetTimespan / 2) {
            diffTime = targetTimespan / 2;
        }
        if (diffTime > targetTimespan * 2) {
    ...
    bool fCheckBind, const Consensus::Params& params)
{
    ...
    if (fNewBest) {
        ...
        // Check bind
        if (miningBlockIndex.nHeight + 1 >= params.xdiskIP006Height) {
        ...
        if (miningBlockIndex.nHeight + 1 >= params.xdiskIP007Height) {
            ...
            if (!privKey)
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                    strprintf("Please pre-set %s private key for mining-sign. The consensus verify at %d.", EncodeDestination(dest), params.xdiskIP007Height));
...
CBlockList GetEvalBlocks(int nHeight, bool fAscent, const Consensus::Params& params)
{
    ...
    CBlockList vBlocks;
    int nBeginHeight = std::max(nHeight - params.nCapacityEvalWindow + 1, params.xdiskIP001PreMiningEndHeight + 1);
...
int64_t GetNetCapacity(int nHeight, const Consensus::Params& params)
{
    ...
    for (const CBlockIndex& block : GetEvalBlocks(nHeight, true, params)) {
        if (nHeight < params.xdiskIP008Height || block.nHeight >= params.xdiskIP008Height) {
    ...
    if (nBlockCount != 0) {
        ...
        if (nBaseTarget != 0) {
            const uint64_t &nInitbaseTarget = nHeight < params.xdiskIP008Height ? xdisk_BASE_TARGET_300 : xdisk_BASE_TARGET_180;
...
{
    if (nHeight < params.xdiskIP008Height) {
        return EvalNetCapacity<xdisk_BASE_TARGET_300>(nHeight, params, associateBlock);
    } else {
        return EvalNetCapacity<xdisk_BASE_TARGET_180>(nHeight, params, associateBlock);
    }
}
...
{
    if (nMiningHeight < params.xdiskIP007Height) {
        // Legacy
        if (pRatioStage) *pRatioStage = -2;

        const arith_uint256 bt240(xdisk_BASE_TARGET_240), bt300(xdisk_BASE_TARGET_300);
        arith_uint256 miningRatio(static_cast<uint64_t>(params.xdiskIP001MiningRatio));
        miningRatio *= bt240;
        miningRatio /= bt300;
        CAmount nLegacyRatio = RoundPledgeRatio(static_cast<CAmount>(miningRatio.GetLow64()));
        return nLegacyRatio;
    } else if (nMiningHeight <= params.xdiskIP007SmoothEndHeight) {
        // Smooth
        ...
        miningRatio /= bt300;
        CAmount nLegacyRatio = RoundPledgeRatio(static_cast<CAmount>(miningRatio.GetLow64()));
        int step = params.xdiskIP007SmoothEndHeight - params.xdiskIP007Height + 1;
        int current = nMiningHeight - params.xdiskIP007Height + 1;
        return RoundPledgeRatio(nLegacyRatio - ((nLegacyRatio - params.xdiskIP001MiningRatio) * current) / step);
    } else {
        // Dynamic
        if (nNetCapacityTB < params.xdiskIP007MiningRatioStage) {
            if (pRatioStage) *pRatioStage = -1;
            return params.xdiskIP001MiningRatio;
        }

        // Range in [0,20]
        if (nNetCapacityTB > params.xdiskIP007MiningRatioStage * 1024 * 1024)
            nNetCapacityTB = params.xdiskIP007MiningRatioStage * 1024 * 1024;
        int nStage = std::max(std::min((int) (std::log2((float) (nNetCapacityTB / params.xdiskIP007MiningRatioStage) + 0.000005f) + 0.000005f), 20), 0);
        assert(nStage <= 20);
        if (pRatioStage) *pRatioStage = nStage;

        CAmount nStartRatio = RoundPledgeRatio((CAmount) (std::pow(0.666667f, (float) nStage) * params.xdiskIP001MiningRatio));
        CAmount nTargetRatio =  RoundPledgeRatio((CAmount) (std::pow(0.666667f, (float) (nStage + 1)) * params.xdiskIP001MiningRatio));
        assert (nTargetRatio > ratio_precise && nStartRatio > nTargetRatio);

        int64_t nStartCapacityTB = (((int64_t)1) << nStage) * params.xdiskIP007MiningRatioStage;
    ...
    int64_t* pRatioCapacityTB, int *pRatioBeginHeight)
{
    ...
    if (nMiningHeight <= params.xdiskIP007SmoothEndHeight) {
        if (pRatioCapacityTB) *pRatioCapacityTB = GetNetCapacity(nMiningHeight - 1, params);
        if (pRatioBeginHeight) *pRatioBeginHeight = std::max(nMiningHeight - params.nCapacityEvalWindow, params.xdiskIP001PreMiningEndHeight);
    } else {
...
// Compatible xdisk007 before consensus
static inline CAmount GetCompatiblePledgeRatio(int nMiningHeight, const Consensus::Params& params)
{
    return nMiningHeight < params.xdiskIP007Height ? params.xdiskIP001MiningRatio : GetMiningRatio(nMiningHeight, params);
}
...
{
    if (nMiningHeight < params.xdiskIP007Height) {
        return EvalNetCapacity<xdisk_BASE_TARGET_240>(nMiningHeight - 1, params, associateBlock);
    } else if (nMiningHeight <= params.xdiskIP008Height) {
        // xdiskIP008 is new genesis block
        return EvalNetCapacity<xdisk_BASE_TARGET_300>(nMiningHeight - 1, params, associateBlock);
    } else {
        return EvalNetCapacity<xdisk_BASE_TARGET_180>(nMiningHeight - 1, params, associateBlock);
    }
}
    ...
    const Consensus::Params& params)
{
    ...
    if (nMiningHeight < params.xdiskIP006BindPlotterActiveHeight) {
        // Mined by plotter ID
...
bool CheckProofOfCapacity(const CBlockIndex& prevBlockIndex, const CBlockHeader& block, const Consensus::Params& params)
{
    ...
    if (prevBlockIndex.nHeight + 1 < params.xdiskIP007Height) {
