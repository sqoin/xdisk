    ...
    uint64_t plotterId, uint64_t nonce, uint64_t deadline, const std::shared_ptr<CKey> privKey)
{
    ...
    if (fundOutIndex < coinbaseTx.vout.size()) {
        coinbaseTx.vout[fundOutIndex].scriptPubKey = GetScriptForDestination(DecodeDestination(chainparams.GetConsensus().xdiskFundAddress));
    ...
    if (plotterId != 0) {
        // Signature
        if (nHeight >= chainparams.GetConsensus().xdiskIP007Height && (!privKey || !privKey->IsValid() || !sign(*pblock, *privKey))) {
