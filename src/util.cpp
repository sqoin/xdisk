...
const char * const BITCOIN_CONF_FILENAME = "xdisk.conf";
const char * const BITCOIN_PID_FILENAME = "xdiskd.pid";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\xdisk
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\xdisk
    // Mac: ~/Library/Application Support/xdisk
    // Unix: ~/.xdisk
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "xdisk";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/xdisk";
#else
    // Unix
    return pathRet / ".xdisk";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good())
        return; // No xdisk.conf file is OK

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override xdisk.conf
