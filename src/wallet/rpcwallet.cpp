...
UniValue setaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nExamples:\n"
            + HelpExampleCli("setaccount", "\"" + Params().GetConsensus().xdiskFundAddress + "\" \"tabby\"")
            + HelpExampleRpc("setaccount", "\"" + Params().GetConsensus().xdiskFundAddress + "\", \"tabby\"")
        );
...
UniValue getaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nExamples:\n"
            + HelpExampleCli("getaccount", "\"" + Params().GetConsensus().xdiskFundAddress + "\"")
            + HelpExampleRpc("getaccount", "\"" + Params().GetConsensus().xdiskFundAddress + "\"")
        );
...
UniValue setprimaryaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\"address\"    (string) The primary BitcoinHD address\n"
            "\nExamples:\n"
            + HelpExampleCli("setprimaryaddress", "\"" + Params().GetConsensus().xdiskFundAddress + "\"")
            + HelpExampleRpc("setprimaryaddress", "\"" + Params().GetConsensus().xdiskFundAddress + "\"")
        );
...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 10)
        throw std::runtime_error(
            ...
            "\nExamples:\n"
            + HelpExampleCli("sendtoaddress", "\"" + Params().GetConsensus().xdiskFundAddress + "\" 0.1")
            + HelpExampleCli("sendtoaddress", "\"" + Params().GetConsensus().xdiskFundAddress + "\" 0.1 \"donation\" \"seans outpost\"")
            + HelpExampleCli("sendtoaddress", "\"" + Params().GetConsensus().xdiskFundAddress + "\" 0.1 \"\" \"\" true")
            + HelpExampleCli("sendtoaddress", "\"[\\\"" + Params().GetConsensus().xdiskFundAddress + "\\\"]\" \"[0.1]\"")
            + HelpExampleRpc("sendtoaddress", "\"" + Params().GetConsensus().xdiskFundAddress + "\", 0.1, \"donation\", \"seans outpost\"")
            + HelpExampleRpc("sendtoaddress", "\"[\\\"" + Params().GetConsensus().xdiskFundAddress + "\\\"]\", [0.1]")
        );
...
UniValue bindplotter(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            "\"txid\"                  (string) The transaction id.\n"
            "\nExamples:\n"
            + HelpExampleCli("bindplotter", "\"" + Params().GetConsensus().xdiskFundAddress + "\" xxxxx")
            + HelpExampleCli("bindplotter", "\"" + Params().GetConsensus().xdiskFundAddress + "\" xxxxx false \"bind\" \"seans outpost\"")
            + HelpExampleCli("bindplotter", "\"" + Params().GetConsensus().xdiskFundAddress + "\" xxxxx true \"\" \"\" true")
            + HelpExampleRpc("bindplotter", "\"" + Params().GetConsensus().xdiskFundAddress + "\", xxxxx, false, \"bind\", \"seans outpost\"")
        );
    ...
    // Check active state
    if (nSpendHeight < params.xdiskIP006Height) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("The bind plotter inactive (Will active on %d)", params.xdiskIP006Height));
    }

    // Fixed bind plotter fee
    if (nSpendHeight >= params.xdiskIP006CheckRelayHeight) {
    ...
    if (nSpendHeight >= params.xdiskIP006LimitBindPlotterHeight) {
        ...
        if (!lastBindInfo.outpoint.IsNull() && nSpendHeight < Consensus::GetBindPlotterLimitHeight(nSpendHeight, lastBindInfo, params)) {
            CAmount diffReward = (GetBlockSubsidy(nSpendHeight, params) * (params.xdiskIP001FundRoyaltyForLowMortgage - params.xdiskIP001FundRoyaltyForFullMortgage)) / 1000;
            if (diffReward > 0) {
                coin_control.m_fee_mode = FeeEstimateMode::FIXED;
                coin_control.fixedFee = std::max(coin_control.fixedFee, diffReward + PROTOCOL_BINDPLOTTER_MINFEE);

                if (!fAllowHighFee)
                    throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("This binding operation triggers a pledge anti-cheating mechanism and therefore requires a large transaction fee %s xdisk. You can enable high bind fee param on call \"bindplotter\"",
...
UniValue unbindplotter(const JSONRPCRequest& request)
{
    ...
    txNew.nLockTime = std::max(nSpendHeight - 1, Params().GetConsensus().xdiskIP006Height);
...
UniValue sendpledgetoaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            "\"txid\"                  (string) The transaction id.\n"
            "\nExamples:\n"
            + HelpExampleCli("sendpledgetoaddress", "\"" + Params().GetConsensus().xdiskFundAddress + "\" 0.1")
            + HelpExampleCli("sendpledgetoaddress", "\"" + Params().GetConsensus().xdiskFundAddress + "\" 0.1 \"donation\" \"seans outpost\"")
            + HelpExampleCli("sendpledgetoaddress", "\"" + Params().GetConsensus().xdiskFundAddress + "\" 0.1 \"\" \"\" true")
            + HelpExampleRpc("sendpledgetoaddress", "\"" + Params().GetConsensus().xdiskFundAddress + "\", 0.1, \"donation\", \"seans outpost\"")
        );
    ...
    if (nAmount <= 0)
        ...
        throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("Small amount for pledge, require more than %s xdisk", FormatMoney(PROTOCOL_RENTAL_AMOUNT_MIN)));
    ...
    // Check active state
    if (chainActive.Height() < Params().GetConsensus().xdiskIP006Height) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("The point to inactive (Will active on %d)", Params().GetConsensus().xdiskIP006Height));
    }
...
UniValue withdrawpledge(const JSONRPCRequest& request)
{
    ...
    CMutableTransaction txNew;
    txNew.nVersion = CTransaction::UNIFORM_VERSION;
    txNew.nLockTime = std::max(chainActive.Height(), Params().GetConsensus().xdiskIP006Height);
