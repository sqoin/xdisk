...
void TxToJSON(const CTransaction& tx, const uint256 hashBlock, UniValue& entry)
{
    ...
    if (tx.IsCoinBase() && pindex != nullptr && pindex->nHeight >= Params().GetConsensus().xdiskIP008Height) {
        if (pindex->nStatus & BLOCK_UNCONDITIONAL) {
            int ratio = Params().GetConsensus().xdiskIP001FundRoyaltyForLowMortgage - GetLowMortgageFundRoyaltyRatio(pindex->nHeight, Params().GetConsensus());
