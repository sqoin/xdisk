...
UniValue getmininginfo(const JSONRPCRequest& request)
{
    ...
    obj.push_back(Pair("smoothbeginheight",  params.xdiskIP007Height));
    obj.push_back(Pair("smoothendheight",    params.xdiskIP007SmoothEndHeight));
    obj.push_back(Pair("stagebeginheight",   params.xdiskIP007SmoothEndHeight + 1));
    obj.push_back(Pair("stagecapacity",      ValueFromCapacity(params.xdiskIP007MiningRatioStage)));
    // Current eval
    ...
    if (chainActive.Height() + 1 > params.xdiskIP007SmoothEndHeight) {
        int nRatioStage = 0;
        ...
        nextEval.push_back(Pair("ratiostartheight", (std::max(chainActive.Height(), params.xdiskIP007SmoothEndHeight) / params.nCapacityEvalWindow + 1) * params.nCapacityEvalWindow));
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
        ...
        fLastTemplateSupportsSegwit != fSupportsSegwit)
    {
        ...
        // Create new block
        CScript scriptDummy = GetScriptForDestination(DecodeDestination(Params().GetConsensus().xdiskFundAddress));
...
UniValue listbindplotterofaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 4)
        throw std::runtime_error(
            ...
            + HelpExampleCli("listbindplotterofaddress", std::string("\"") + Params().GetConsensus().xdiskFundAddress + "\" \"0\" 10")
            + HelpExampleRpc("listbindplotterofaddress", std::string("\"") + Params().GetConsensus().xdiskFundAddress + "\", \"0\" 10")
        );
...
UniValue createbindplotterdata(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "\nExamples:\n"
            "\nReturn bind plotter hex data\n"
            + HelpExampleCli("createbindplotterdata", std::string("\"") + Params().GetConsensus().xdiskFundAddress + "\" \"root minute ancient won check dove second spot book thump retreat add\"")
            + HelpExampleRpc("createbindplotterdata", std::string("\"") + Params().GetConsensus().xdiskFundAddress + "\", \"root minute ancient won check dove second spot book thump retreat add\"")
        );
    ...
    {
        LOCK(cs_main);
        activeHeight = std::max(chainActive.Height(), Params().GetConsensus().xdiskIP006Height);
    }
...
UniValue verifybindplotterdata(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            ...
            + HelpExampleCli("verifybindplotterdata", std::string("\"") + Params().GetConsensus().xdiskFundAddress + "\" \"0000\"")
            + HelpExampleRpc("verifybindplotterdata", std::string("\"") + Params().GetConsensus().xdiskFundAddress + "\", \"0000\"")
        );
    ...
    {
        LOCK(cs_main);
        nHeight = std::max(chainActive.Height(), Params().GetConsensus().xdiskIP006Height);
    }
    bool fReject = false;
    int lastActiveHeight = 0;
...
UniValue GetPledge(const std::string &address, uint64_t nPlotterId, bool fVerbose)
{
    ...
    if (chainActive.Height() + 1 < params.xdiskIP006BindPlotterActiveHeight) {
...
UniValue getpledgeofaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. address         (string, required) The BitcoinHD address.\n"
            "2. plotterId       (string, optional) DEPRECTED after xdiskIP006. Plotter ID\n"
            ...
            "]\n"
            "\nExample:\n"
            + HelpExampleCli("getpledgeofaddress", std::string("\"") + Params().GetConsensus().xdiskFundAddress + "\" \"0\" true")
            + HelpExampleRpc("getpledgeofaddress", std::string("\"") + Params().GetConsensus().xdiskFundAddress + "\", \"0\", true")
            );
...
UniValue getplottermininginfo(const JSONRPCRequest& request)
{
    ...
    if (!vBlocks.empty()) {
        ...
        for (const CBlockIndex& block : vBlocks) {
            ...
            if (chainActive.Height() < params.xdiskIP008Height || block.nHeight >= params.xdiskIP008Height) {
    ...
    if (chainActive.Height() < params.xdiskIP006BindPlotterActiveHeight) {
        // Mined by plotter ID
        if (nCapacityTB > 1) {
            typedef struct {
                int forgeCount;
...
UniValue listpledgeloanofaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            + HelpExampleCli("listpledgeloanofaddress", std::string("\"") + Params().GetConsensus().xdiskFundAddress + "\"")
            + HelpExampleRpc("listpledgeloanofaddress", std::string("\"") + Params().GetConsensus().xdiskFundAddress + "\"")
        );
...
UniValue listpledgedebitofaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            + HelpExampleCli("listpledgedebitofaddress", std::string("\"") + Params().GetConsensus().xdiskFundAddress + "\"")
            + HelpExampleRpc("listpledgedebitofaddress", std::string("\"") + Params().GetConsensus().xdiskFundAddress + "\"")
        );
...
UniValue getbalanceofheight(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\n"
            "\nExample:\n"
            + HelpExampleCli("getbalanceofheight", Params().GetConsensus().xdiskFundAddress + " 9000")
            + HelpExampleRpc("getbalanceofheight", std::string("\"") + Params().GetConsensus().xdiskFundAddress + "\", 9000")
            );
