...
UniValue blockheaderToJSON(const CBlockIndex* blockindex)
{
    ...
    if (blockindex->nHeight >= Params().GetConsensus().xdiskIP007Height) {
...
UniValue listunspentofaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nExamples:\n"
            + HelpExampleCli("listunspentofaddress", std::string("\"") + Params().GetConsensus().xdiskFundAddress + "\"")
            + HelpExampleRpc("listunspentofaddress", std::string("\"") + Params().GetConsensus().xdiskFundAddress + "\"")
        );
