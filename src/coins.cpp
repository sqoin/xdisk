...
void AddCoins(CCoinsViewCache& cache, const CTransaction &tx, int nHeight, bool check) {
    ...
    if (nHeight >= Params().GetConsensus().xdiskIP006Height)
...
bool CCoinsViewCache::SpendCoin(const COutPoint &outpoint, Coin* moveout, bool rollback) {
    ...
    if (!rollback && it->second.coin.IsBindPlotter() && it->second.coin.nHeight >= Params().GetConsensus().xdiskIP007Height) {
...
CBindPlotterInfo CCoinsViewCache::GetChangeBindPlotterInfo(const CBindPlotterInfo &sourceBindInfo, bool compatible) const {
    ...
    if (compatible) {
        // Compatible xdiskIP007 before. Use last active coin
